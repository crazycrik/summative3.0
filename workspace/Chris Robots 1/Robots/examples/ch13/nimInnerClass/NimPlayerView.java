...
{	
   ...
   private JButton[] removeButtons = new JButton[] {
      new JButton("Remove 1 Token"), 
      new JButton("Remove 2 Tokens"),
   ...
   {  Player whoseTurn = this.model.getWhoseTurn();
      ...
      {  this.removeButtons[i].setEnabled(
               whoseTurn == this.player && i + 1 <= pSize);
      }
   ...
   {  RemoveButtonController controller = new RemoveButtonController();
   	for (int i = 0; i < this.removeButtons.length; i++)
      {  this.removeButtons[i].addActionListener(controller);
      }
   }
   
   
   private class RemoveButtonController extends Object implements ActionListener
   {	public void actionPerformed(ActionEvent evt)
   	{	JButton src = (JButton)evt.getSource();
   		/*
   		if (src == removeButtons[0])
   		{	model.removeTokens(1);
   		} else if (src == removeButtons[1])
   		{	model.removeTokens(2);
   		} else if (src == removeButtons[2])
   		{	model.removeTokens(3);
   		} else
