...
public class NimModel extends AbstractModel
		// Object implements IModel	// Not necessary if we extend AbstractModel
{
	// Not necessary if we extend AbstratModel
   ...
   /** Remove one, two or three tokens from the pile.  Ignore any attempts to
...
//
//	These methods can be inherited from AbstractModel.
//
...
//   }
   ...
   {  System.out.println("Testing NimModel");
      ...
      Test.ckEquals("pilesize", 10, nim.getPileSize());
      Test.ckEquals("winner", Player.NOBODY, nim.getWinner());
      Test.ckEquals("next", Player.RED, nim.getWhoseTurn());
      
      nim.removeTokens(3);
      Test.ckEquals("pilesize1", 7, nim.getPileSize());
      Test.ckEquals("winner1", Player.NOBODY, nim.getWinner());
      Test.ckEquals("next1", Player.BLACK, nim.getWhoseTurn());
      
      nim.removeTokens(2);
      Test.ckEquals("pilesize2", 5, nim.getPileSize());
      Test.ckEquals("winner2", Player.NOBODY, nim.getWinner());
      Test.ckEquals("next2", Player.RED, nim.getWhoseTurn());
      
      nim.removeTokens(3);
      Test.ckEquals("pilesize3", 2, nim.getPileSize());
      Test.ckEquals("winner3", Player.NOBODY, nim.getWinner());
      Test.ckEquals("next3", Player.BLACK, nim.getWhoseTurn());
      
      nim.removeTokens(2);
      Test.ckEquals("pilesize4", 0, nim.getPileSize());
      Test.ckEquals("winner4", Player.BLACK, nim.getWinner());
      Test.ckEquals("next4", Player.NOBODY, nim.getWhoseTurn());
      ...
      System.out.println(
            "\nDone testing.  " + Test.getNumErrors() + " failures.");
   }
