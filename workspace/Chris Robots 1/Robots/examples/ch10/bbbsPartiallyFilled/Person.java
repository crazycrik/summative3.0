...
{  private String name;			// person's name
   ...
   private static final int NUM_INTERESTS = 4;
   private double[] interests = new double[NUM_INTERESTS];
   ...
   {  super();
      ...
      // Read this person's interests from the file.
      for (int i=0; i<Person.NUM_INTERESTS; i++)
      {  this.interests[i] = in.nextDouble();
      }
      in.nextLine();
   ...
   {  if (this.isPaired() || p.isPaired() || this.role == p.role
      ...
      double compat = 0.0;
      for (int i=0; i<Person.NUM_INTERESTS; i++)
      {  compat = compat + this.interests[i] * p.interests[i];
      }
      return compat / Person.NUM_INTERESTS;
   }
   ...
   {	
      ...
      {  p = p + "Paired with: " + this.pairName + "\n";
		...
		for(int i=0; i<Person.NUM_INTERESTS; i++)
		{	p = p + " " + this.interests[i];
		}
		p = p + System.getProperty("line.seprator");
      return p;
   }
   ...
   {  out.println(this.name);
      ...
      for(int i=0; i<Person.NUM_INTERESTS; i++)
      {	out.print(nf.format(this.interests[i]) + " ");
      }
